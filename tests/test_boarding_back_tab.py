...
naming it after they beam down. So the PADD's Boarding Party screen declared a back tab to it,
and pressing Back walked a crew member to Helm while their character was standing on a
planet.

It did that by jumping the tab route, which is not the way up. `boarding_go_up` releases the
...
why the back tab must not be offered), and `boarding_go_up` really does clean up (which is
...
class TestWhileOnTheSurface(AwayBase):
    ...
    def test_AND_THE_SCREEN_DECLARES_NO_BACK_TAB(self):
        """The expression the PADD screen evaluates: `if not boarding_who(client_id)`."""
        self.send_down()
