...
class InviteBase(unittest.TestCase):
    ...
    def invite(self, roster=None):
        return A.boarding_invite(1, roster if roster is not None else self.roster,
                             title="The Shed")
...
class TestBeamingDown(InviteBase):
    ...
    def test_a_console_can_simply_say_yes(self):
        ...
        got = A.boarding_beam_down(HELM)
        self.assertEqual(got, 101)
        self.assertEqual(A.boarding_me(HELM), 101)

    def test_or_pick_somebody(self):
        self.assertEqual(A.boarding_beam_down(HELM, 103), 103)
        self.assertEqual(A.boarding_me(HELM), 103)

    def test_a_character_who_is_taken_is_no_longer_offered(self):
        A.boarding_beam_down(HELM, 102)
        self.assertEqual(A.boarding_open_roster(), [101, 103])

    def test_two_consoles_cannot_take_the_same_character(self):
        A.boarding_beam_down(HELM, 101)
        self.assertIsNone(A.boarding_beam_down(WEAP, 101))
        self.assertEqual(A.boarding_me(WEAP), None)
    ...
    def test_the_party_can_run_out(self):
        ...
        for cid in (HELM, WEAP, SCI):
            self.assertIsNotNone(A.boarding_beam_down(cid))
        self.assertIsNone(A.boarding_beam_down(11))
        self.assertEqual(A.boarding_open_roster(), [])

    def test_nobody_can_join_without_an_invitation(self):
        A.boarding_invite_clear()
        self.assertIsNone(A.boarding_beam_down(HELM))

    def test_a_closed_invitation_takes_no_more(self):
        A.boarding_beam_down(HELM)
        A.boarding_invite_close()
        self.assertIsNone(A.boarding_beam_down(WEAP))
...
class TestBeamingUp(InviteBase):
    ...
    def test_leaving_releases_the_character(self):
        self.assertTrue(A.boarding_beam_up(HELM))
        self.assertIsNone(A.boarding_me(HELM))
        self.assertEqual(A.boarding_team(), set())

    def test_and_somebody_else_can_take_them(self):
        A.boarding_beam_up(HELM)
        self.assertIn(101, A.boarding_open_roster())
        self.assertEqual(A.boarding_beam_down(WEAP, 101), 101)

    def test_leaving_when_you_are_not_down_is_not_an_error(self):
        self.assertFalse(A.boarding_beam_up(WEAP))
...
class TestReset(InviteBase):
    def test_the_invitation_does_not_survive_the_mission(self):
        ...
        A.boarding_clear()
        self.assertIsNone(A.boarding_invitation())
        self.assertEqual(A.boarding_invite_count(), 0)
    ...
    def test_it_is_on_the_reset_ledger(self):
        ...
        self.assertIn("boarding invitation", _RESET_PROBES)



class TestTheConsoleHalf(InviteBase):
    """`boarding_beam_down` takes a character; `boarding_go_down` turns the console into
    ...
    def setUp(self):
        ...
        from sbs_utils.procedural.gui import boarding_gui
        self.gui = boarding_gui
        GuiClient(HELM)
        self.invite()
    ...
    def test_it_remembers_the_post_to_come_back_to(self):
        ...
        A.boarding_beam_down(HELM)
        self.gui.boarding_go_down(HELM)
        self.assertEqual(self.value("BOARDING_RETURN"), "science")
    ...
    def test_coming_back_restores_that_post(self):
        ...
        A.boarding_beam_down(HELM)
        self.gui.boarding_go_down(HELM)
        self.assertTrue(self.gui.boarding_go_up(HELM))
        self.assertEqual(self.value("CONSOLE_TYPE"), "science")
        self.assertFalse(has_role(HELM, "boarding"),
                         "the away role outlived the crew console")

    def test_coming_back_frees_the_character(self):
        A.boarding_beam_down(HELM, 102)
        self.gui.boarding_go_down(HELM)
        self.gui.boarding_go_up(HELM)
        self.assertIn(102, A.boarding_open_roster())

    def test_going_down_without_a_character_does_nothing(self):
        self.assertFalse(self.gui.boarding_go_down(HELM))

    def test_who_this_console_is(self):
        A.boarding_beam_down(HELM, 103)
        self.assertEqual(self.gui.boarding_who(HELM), 103)

    def test_a_console_speaking_for_two_can_choose_which(self):
        """The gap that stopped the PADD replacing the crew console: the deduped
        choice list collapses shared options onto the primary."""
        A.boarding_beam_down(HELM, 101)
        A.boarding_assign_also(HELM, 102)
        self.assertEqual(self.gui.boarding_who(HELM), 101)
        self.gui.boarding_set_who(HELM, 102)
        self.assertEqual(self.gui.boarding_who(HELM), 102)

    def test_it_cannot_be_set_to_somebody_this_console_does_not_hold(self):
        A.boarding_beam_down(HELM, 101)
        self.gui.boarding_set_who(HELM, 103)
        self.assertEqual(self.gui.boarding_who(HELM), 101)
...
class TestTheAppOnlyExistsWhenItIsUseful(InviteBase):
    ...
    def setUp(self):
        super().setUp()
        from sbs_utils.gui import GuiClient
        from sbs_utils.procedural.gui.boarding_gui import boarding_relevant
        from sbs_utils.procedural.gui import boarding_gui
        self.gui = boarding_gui
        self.relevant = boarding_relevant
        GuiClient(HELM)
