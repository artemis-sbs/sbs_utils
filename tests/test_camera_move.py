...
from sbs_utils.procedural.gui.camera import (
    ...
    camera_lens, camera_shot, _ease, _MOVES)
...
class TestMove(MoveBase):
    def test_the_lens_travels_from_a_to_b(self):
        ...
        self.advance(3.0)
        self.assertAlmostEqual(self.lens_of(C1).z, -1000, places=0)
    ...
    def test_it_always_lands_exactly_on_the_end(self):
        ...
        self.advance(10.0)
        self.assertAlmostEqual(self.lens_of(C1).z, -500, places=0)
    ...
    def test_it_follows_a_moving_subject(self):
        ...
        self.assertAlmostEqual(self.lens_of(C1).x, 0, places=0)
    ...
    def test_a_second_move_replaces_the_first(self):
        ...
        self.advance(0.2)
        self.assertLess(abs(self.lens_of(C1).z + 1000), 200)
    ...
    def test_moves_on_different_consoles_are_independent(self):
        ...
        self.advance(1.0)
        self.assertAlmostEqual(self.lens_of(C1).z, -4000, places=0)
        self.assertAlmostEqual(self.lens_of(C2).z, -800, places=0)
...
class TestOrbit(MoveBase):
    def test_it_holds_its_distance(self):
        ...
        for _ in range(8):
            self.advance(0.5)
            lens = self.lens_of(C1)
            seen.append((lens.x ** 2 + lens.y ** 2 + lens.z ** 2) ** 0.5)
        for d in seen:
    ...
    def test_it_actually_goes_round(self):
        ...
        first = self.lens_of(C1)
        self.advance(4.5)
        last = self.lens_of(C1)
    ...
    def test_it_orbits_a_moving_subject(self):
        ...
        self.advance(1.0)
        lens = self.lens_of(C1)
        d = ((lens.x - 9000) ** 2 + lens.y ** 2 + lens.z ** 2) ** 0.5
...
class TestRack(MoveBase):
    def test_it_re_aims_without_moving_the_lens(self):
        ...
        self.advance(0.1)
        before = self.lens_of(C1)
        camera_rack(C1, b)
        after = self.lens_of(C1)
    ...
    def test_it_stops_a_running_move(self):
        ...
        self.advance(0.2)
        camera_rack(C1, b)
        held = self.lens_of(C1)
        self.advance(3.0)
        self.assertAlmostEqual(self.lens_of(C1).z, held.z, places=0,
...
class TestStopAndState(MoveBase):
    def test_stop_leaves_the_lens_where_it_is(self):
        ...
        self.advance(1.0)
        where = self.lens_of(C1)
        camera_move_stop(C1)
        self.advance(3.0)
        self.assertAlmostEqual(self.lens_of(C1).z, where.z, places=0)

    def test_camera_eye_reports_the_lens(self):
        subj = camera_anchor(0, 0, 0)
        camera_move(C1, subj, (0, 0, -2500), (0, 0, -2500), 2.0)
        self.advance(0.2)
        self.assertAlmostEqual(camera_lens(C1).z, -2500, places=0)
