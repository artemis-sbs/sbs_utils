...
|---|---|---|---|---|
| 1 | LM | `documents/quest_tab.mast:47` | `qbox` | `collapsible=True` + `set_selected_index(i, False)` + repaint-on-select — **structurally identical to the gallery index**, which is already engine-verified |
...
**Start with #1.** Same widget configuration as the proven caller, so if it
behaves differently the difference is the caller, not the feature.
...
|---|---|---|---|
| LM | `hangar/hangar.mast:156` | `dock_picker` | **11 jumps to `show_hangar`**, plus `//gui/tab` |
| LM | `hangar/hangar.mast:157` | `ride_picker` | same page |
| LM | `hangar/hangar.mast:188` | `quest_box` | same page |
