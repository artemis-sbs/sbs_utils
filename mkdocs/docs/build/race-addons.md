...
!!! warning "List this in your `story.json`"
    ...
    console**, and **nothing raids you** â€” with no error to tell you why.
...
|---|---|---|
| **Ship interiors** | one ASCII floor plan per hull â€” the rooms and system nodes Engineering shows | the Engineering console is empty: no system nodes, no damcons, no internal damage |
| **Fleet ladders** | which ships make up a raiding fleet at each difficulty | `fleet_create` finds no ladder, so nothing spawns |

Nine races in v1.4.0 â€” TSN, Ximni, USFP, Arvonian, Torgoth, Skaraan, Kralien, Biomech and
Pirate â€” covering **63 ship interiors** and **six fleet ladders**.
...
`random.choice([...])` beside it â€” so adding a race meant editing a mission library, and a
...
A race is a block in `races/__init__.mast`, or a whole add-on of your own â€” the calls are
...
- [Fleets & raiding](fleets.md) â€” ladders, difficulty, spawning
- [Making a mod](making-a-mod.md) â€” a race with its own ships and art
- [Making add-ons](addons.md)
- [Mission settings](../home/settings.md)
- [Damage](damage.md) â€” what the interior's system nodes actually do
