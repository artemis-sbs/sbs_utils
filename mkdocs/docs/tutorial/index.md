...
```
Go ahead and add at least one more button with some more scan information to your own. This is a good time to remind you, if you haven't figured it out already, that python, and MAST, use indentation to designate blocks of code. Note that the button, scan, and scan text sections are seqeuentially indented. All of these scan buttons should have the same indentation.
...
//damage/object if has_role("treasure")
    delete_object(DAMAGE_SELECTED_ID)
```
We've now gotten rid of our asteroid. How do we replace it with treasure? What should our treasure be? I suppose its time we figured that out. I'm thinking, for simiplicity's sake, that the treasure is an upgrade. How about a Tauron Focuser?
