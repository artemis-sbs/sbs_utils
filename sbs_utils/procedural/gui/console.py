...
def gui_console_enter(client_id, console_type, ship=None):
    ...
    **It fires on a CHANGE of console type, not on a repaint.** A screen is
    ...
    """
    from ..crew import crew_assign, crew_post_of
    from ..inventory import set_inventory_value
    from ..query import to_id
    from ..roles import add_role, remove_role
    from .camera import camera_assign, camera_auto
    from .console_types import gui_get_console_types
    from .overlay import overlay_clear_console
    from .viewscreen import viewscreen_home_ship, viewscreen_restore
    from .viewscreen_claims import viewscreen_owner
    from .widgets import gui_widget_list_clear

    was = get_inventory_value(client_id, "CONSOLE_TYPE", None)
    if was == console_type:
        return False

    home = to_id(ship) if ship is not None else viewscreen_home_ship(client_id)

    overlay_clear_console(client_id)

    # Only OUR claim. A per-console owner token ends in ":<client_id>"; a bare one
    # ("hail", "docking") belongs to the ship and outlives this console's move.
    if home and viewscreen_owner(home).endswith(":%s" % client_id):
        viewscreen_restore(home)
    ...
    if post is not None and home:
        pick = "%s:%s" % (post.roster, post.key) if post.roster and post.key else None
        crew_assign(client_id, home, console_type, own_pick=pick)
