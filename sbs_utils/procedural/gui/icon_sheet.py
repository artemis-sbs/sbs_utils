...
def icon_resolve(name):
    ...
    The atlas branch is what makes a custom sheet a drop-in later: register the name with
    `gui_image_add_atlas` and it wins, with no change to anything that draws it.
    """
    ...
    while key in ICON_ALIAS and key not in seen:
        seen.add(key)
        key = ICON_ALIAS[key]
    if key in ImageAtlas.all:            # a custom sheet claimed this name
        return None, key
