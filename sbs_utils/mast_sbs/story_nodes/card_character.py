...
@mast_node()
class CharacterCardLabel(CardLabelBase):
    rule = re.compile(r'@character/(?P<path>([\w/]+))[ /t]*'+STRING_REGEX_NAMED("display_name")+IF_EXP_REGEX)

    def __init__(self, path, display_name=None, q=None, if_exp=None, loc=None, compile_info=None):
        super().__init__("character", path, display_name, if_exp, loc, compile_info)
