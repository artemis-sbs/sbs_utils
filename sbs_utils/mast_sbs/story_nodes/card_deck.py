...
@mast_node()
class DeckCardLabel(CardLabelBase):
    rule = re.compile(r'@deck/(?P<path>([\w/]+))[ /t]*'+STRING_REGEX_NAMED("display_name")+IF_EXP_REGEX)

    def __init__(self, path, display_name=None, q=None, if_exp=None, loc=None, compile_info=None):
        super().__init__("deck", path, display_name, if_exp, loc, compile_info)
